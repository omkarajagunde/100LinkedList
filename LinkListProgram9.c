...
// This function will search a particular element's last
// occurence and return its position ...
int searchFirstOcc(SLLNode *headNode, int searchElem){
	...
	if (lastOcc > 0 && lastOcc <= countNodes)
		return lastOcc;
	else
		return -1;
}
...
void main(void){
	...
	do{
		...
		printf("3] Search Element's last occurence ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 3: printf("\nEnter a element to search ? ");
				scanf("%d",&data);
				retValue = searchFirstOcc(headNode, data);
				if (retValue == -1) { 
