...
// This function will the Last node of CLList ...
int deleteAtPosition(SLLNode **headNode, SLLNode **tailNode, int pos){
    ...
    if (pos == 1){
        deleteFirstPosition(headNode, tailNode);
        return 0;
    }

    if (pos == countNodes(&(*headNode),&(*tailNode)) - 1){
        deleteLastPosition(headNode, tailNode);    
        return 0;
    }
	...
	do{
		...
		switch(choice){
            ...
            printf("Enter a position to delete Node :");
                scanf("%d",&pos);
                retValue = deleteAtPosition(&headNode, &tailNode, pos);
                if (retValue == -1)
                    printf(CYN"\t\tNo nodes Present to delete ..."RESET);
                break;
