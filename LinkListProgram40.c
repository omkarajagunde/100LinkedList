...
// This function prints the middle of node ...
int displayMiddleNode(SLLNode *headNode){

    SLLNode *dummyNode = headNode;
    int totalNodes = countNodes(&headNode);
    if (totalNodes % 2 == 0 || totalNodes == 1)
        return -1;

    int count = totalNodes / 2;
    int i = 0;
    ...
    printf(CYN"\t\tMiddle Data -> %d ..."RESET, dummyNode -> data);
    return 0;
}
	...
	do{
		...
		switch(choice){
			...
			case 2:	displayNodes(&headNodeListOne);
				break;
			case 3: 
                retValue = displayMiddleNode(headNodeListOne);
                if (retValue == -1)
                    printf(CYN"\t\tEven total nodes count does not have middle nodes ..."RESET);
				break;
