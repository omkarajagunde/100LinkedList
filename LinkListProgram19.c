...
// This function will copy the start-end range nodes from source list to
// destination list ...
int llCopyRange(SLLNode **srcList, SLLNode **destList, int start, int end){

	if (*srcList == NULL) return -1;
	SLLNode *destCpyList = *srcList;
    SLLNode *destCpyEnd = *srcList;
    SLLNode *traverseNode;
    int count = 1;
    ...
    traverseNode = destCpyList;

    count = 1;
    while (count != end){
        destCpyEnd = destCpyEnd -> nextNode;
        count += 1;
    }
    destCpyEnd -> nextNode = NULL;
	...
	do{
		...
		switch(choice){
			...
			case 2:	printf("\n\tView list 1 or 2 [1/2] :");
				...
				break;
			case 3: printf("Enter start ? :");
                scanf("%d", &start);
                printf("Enter end ? :");
                scanf("%d", &end);
                headNodeListTwo = NULL;
                llCopyRange(&headNodeListOne,&headNodeListTwo,start,end); 
				break;
