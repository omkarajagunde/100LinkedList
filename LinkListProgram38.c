...
// This function checks whether the 2 linked list are similar or not upto given position...
int listCompare(SLLNode *srcList, SLLNode *destList, int pos){
...
void main(void){
	...
	do{
		...
		printf("4] Check both link list are same or not upto given node ? :\n");
		printf("5] Exit :\n");
		...
		switch(choice){
			...
			case 4: 
                ...
                scanf("%d",&data);
                retValue = listCompare(headNodeListOne, headNodeListTwo, data);   
                if (retValue == -1)
                    printf(CYN"\t\tBoth lists are Not equal..."RESET);
                if (retValue == 0)
                    printf(CYN"\t\tBoth lists are equal till %d..."RESET,data);
				break;
