...
// in number is even or odd ...
int checkDigitAdditionEven(int num){
...
// This function will copy the even addition data nodes from source list to
// destination list ...
int llCopyEx(SLLNode **srcList, SLLNode **destList){
	...
	while (destCpyList != NULL){
        ...
        if (dummyNode == NULL){
            if (checkDigitAdditionEven(destCpyList -> data)){
...
void main(void){
	...
	do{
		...
		printf("3] Copy even addition digit nodes of List 1 @ List 2 ? :\n");
		printf("4] Exit :\n");
