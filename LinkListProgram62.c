...
// This function will insert a Element specified position of the DLList ...
int insertAtPos(SLLNode **headNode, int data, int position){
	...
	if (position == 1){
		...
		*headNode = newNode;
		...
		int traverse = 2;
		while (traverse != position){
			dummyNode = dummyNode -> nextNode;
			traverse += 1;	
		}
	
		newNode -> nextNode = dummyNode -> nextNode -> nextNode;
        dummyNode -> nextNode -> nextNode -> prevNode = newNode;
		dummyNode -> nextNode = newNode;
        newNode -> prevNode = dummyNode;
...
void main(void){
	...
	do{
		...
		printf("3] Insert @ specific position in DLList 1 ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 3: 
                ...
                scanf("%d",&pos);
                retValue = insertAtPos(&headNodeListOne, data, pos);
                if (retValue == 0)
                    printf(CYN"\t\tNode added Successfully ..."RESET);
				break;
