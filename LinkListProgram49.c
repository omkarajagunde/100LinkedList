...
int searchLastOcc(SLLNode **headNode, SLLNode **tailNode, int data){
...
void main(void){
	...
	do{
		...
		printf("3] Search last occurence of element? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
            ...
            printf("Enter data to find last Occurence ? :");
                scanf("%d",&data);
                retValue = searchLastOcc(&headNode, &tailNode, data);
                if (retValue == -1)
                    printf(CYN"\t\tData entered was not found ..."RESET);
                else
                    printf(CYN"\t\tLast occurence of data present at position -> %d"RESET, retValue);
