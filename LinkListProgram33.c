...
// This function checks whether the 2 linked list intersect each other or not ...
int listIntersect(SLLNode **srcList, SLLNode **destList){
...
void main(void){
	...
	do{
		...
		printf("4] Check is there Intersection in 2 lists ? :\n");
		printf("5] Exit :\n");
		...
		switch(choice){
			...
			case 3:	displayNodes(&headNodeListOne);
				...
				break;
			case 4: 
                headNodeListOne -> nextNode -> nextNode -> nextNode = headNodeListTwo -> nextNode -> nextNode; 
                retValue = listIntersect(&headNodeListOne, &headNodeListTwo);   
                if (retValue == -1)
                    printf(CYN"\t\tList Intersection Found ... Nodes were hardCoded as \n-> headNodeListOne -> nextNode -> nextNode -> nextNode = headNodeListTwo -> nextNode -> nextNode"RESET);
                if (retValue == 0)
                    printf(CYN"\t\tList Intersection Not Found ..."RESET);
				break;
