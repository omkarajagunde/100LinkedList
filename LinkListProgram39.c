...
// This function checks whether the 2 linked list are similar for last N nodes...
int listCompareLastN(SLLNode *srcList, SLLNode *destList, int pos){

    SLLNode *srcCpy = srcList;
    SLLNode *destCpy = destList;
    int count = 0;
    int srcListCount = countNodes(&srcList) - pos;
    int destListCount = countNodes(&destList) - pos;

    if (srcList == NULL && destList == NULL)
        return 2;

    while (count != srcListCount){
        srcCpy = srcCpy -> nextNode;
        count ++;
    }

    count = 0;
    while (count != destListCount){
...
void main(void){
	...
	do{
		...
		printf("4] Check both link list's last n nodes are same or not ? :\n");
		printf("5] Exit :\n");
		...
		switch(choice){
			...
			case 3:	displayNodes(&headNodeListOne);
				...
				break;
			case 4: 
                printf("Enter a position till which node you want to check ? :");
                scanf("%d",&data);
                retValue = listCompareLastN(headNodeListOne, headNodeListTwo, data);   
                if (retValue == -1)
                    printf(CYN"\t\tBoth lists are Not equal..."RESET);
                if (retValue == 0)
                    printf(CYN"\t\tBoth lists's last %d are equal ..."RESET,data);
				break;
