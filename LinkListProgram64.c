...
// This function will remove the first element of the DLList ...
int removeLast(SLLNode **headNode){
...
void main(void){
	...
	do{
		...
		printf("3] Delet last node in DLList 1 ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 2:	displayNodes(&headNodeListOne);
				break;
			case 3: 
                retValue = removeLast(&headNodeListOne);
                if (retValue == 0)
                    printf(CYN"\t\tLast Node removed Successfully ..."RESET);
                if (retValue == -1)    
