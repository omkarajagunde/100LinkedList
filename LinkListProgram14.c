...
// This function will concat the 2nd Link list's last n nodes to the 1st Link List ..
int concatLastN(SLLNode **sourceList, SLLNode **destList,int n){

	SLLNode *sourceListCopy = *sourceList;
	SLLNode *dummyNode = *destList;
	int count = countNodes(&(*destList)) - n;
	int i = 0;
	...
	sourceListCopy -> nextNode = dummyNode;
	return 0;
	...
	do{
		...
		switch(choice){
			...
			case 3:	printf("\n\tView list 1 or 2 [1/2] :");
				...
				break;
			case 4: if(attempt == 1){
					printf("Enter how many elements to concat ? :");
					scanf("%d",&data);
					concatLastN(&headNodeListTwo,&headNodeListOne,data); 
				attempt += 1;
				}else
