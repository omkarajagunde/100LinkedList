...
// This function will will delete the last node from the linked list of nodes ...
int deleteAtPosition(SLLNode **headNode, int position){
	...
	}else if (position > 1 && position < countNodes){
	...
	do{
		...
		switch(choice){
			...
			case 3: printf("\nEnter a position to delete :");
				scanf("%d",&position);
				deleteAtPosition(&headNode, position);
				break;
