...
// This function performs consecutive swap on the Linked List ...
int alternateNodes(SLLNode **srcList, SLLNode **destList){

    SLLNode *dummyNode = *srcList;

    SLLNode *headOne = *srcList;
    SLLNode *nodeOne = headOne;

    SLLNode *headTwo = *destList;
    SLLNode *nodeTwo = headTwo;

    while (headTwo != NULL){

        headOne = nodeOne -> nextNode;
        headTwo = nodeTwo -> nextNode;

        nodeOne -> nextNode = nodeTwo;
        nodeTwo -> nextNode = headOne;

        nodeOne = headOne;
        nodeTwo = headTwo;

        if (headOne -> nextNode == NULL && headTwo != NULL){
            headOne -> nextNode = headTwo;
            return 0;
        }
...
void main(void){
	...
	do{
		...
		printf("4] Check is there loop or not ? :\n");
		printf("5] Exit :\n");
		...
		switch(choice){
			...
			case 3:	displayNodes(&headNodeListOne);
				break;
			case 4: 
                alternateNodes(&headNodeListOne, &headNodeListTwo);   
				break;
