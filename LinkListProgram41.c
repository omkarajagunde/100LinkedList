...
// This function adds a node at the first position of Linked List ...
int insertAtFirst(SLLNode **headNode, SLLNode **tailNode, int data){
...
void main(void){
	...
	do{
		...
		printf("3] Add node at First of List 1? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 2:	displayNodes(&headNode, &tailNode);
				break;
			case 3: 
                printf("Enter a data to add @ first position : ");
                scanf("%d",&data);
                insertAtFirst(&headNode, &tailNode, data);
                break;
