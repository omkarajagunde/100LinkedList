...
// This function will insert a Element to the End of the DLList ...
int insertLast(SLLNode **headNode, int data){

    SLLNode *tempNode = createNode(data);
    SLLNode *dummyNode = *headNode;
    while (dummyNode -> nextNode != NULL){
        dummyNode = dummyNode -> nextNode;
    }

    dummyNode -> nextNode = tempNode;
    tempNode -> prevNode = dummyNode;
...
void main(void){
	...
	do{
		...
		printf("3] Insert @ Last position in DLList 1 ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 3: 
                ...
                scanf("%d",&data);
                retValue = insertLast(&headNodeListOne, data);
				break;
