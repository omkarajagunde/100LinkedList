...
// This function performs palindrome check on the Linked List ...
int listPalindromeCheck(SLLNode **srcList){

    SLLNode *srcCpy = NULL;
    SLLNode *dummyNode = *srcList;
    llCopy(srcList, &srcCpy);
    reverseList(&srcCpy);

    int flag = 0;
    while (dummyNode != NULL){
        if (dummyNode -> data != srcCpy -> data){
            flag = 1;
            break;
        }
        dummyNode = dummyNode -> nextNode;
...
void main(void){
	...
	do{
		...
		printf("3] Check if List 1 is palindrome or not ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 2:	displayNodes(&headNodeListOne);
				break;
			case 3: 
                retValue = listPalindromeCheck(&headNodeListOne);   
                if (retValue == 0)
                    printf(CYN"\t\tList is palindrome ..."RESET);
                else
                    printf(CYN"\t\tList is not palindrome ..."RESET);
