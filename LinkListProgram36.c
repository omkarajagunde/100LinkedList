...
// This function makes a copy of linked list ...
...
// This function will sort Linked List in Ascending Order ...
int listSortDsc(SLLNode **srcList){

    SLLNode *dummyNode = *srcList;
    while (dummyNode -> nextNode != NULL){

        SLLNode *tempNode = dummyNode;
        SLLNode *aheadNode = dummyNode -> nextNode;

        while (aheadNode != NULL){

            if (aheadNode -> data > tempNode -> data){
                tempNode = aheadNode;
            }
        ...
        tempNode -> data = dummyNode -> data;
        dummyNode -> data = tempData;
...
void main(void){
	...
	do{
		...
		printf("3] Show Link list 1 is Descending order ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 2:	displayNodes(&headNodeListOne);
				break;
			case 3: 
                retValue = listSortDsc(&headNodeListOne);   
                printf(CYN"\t\tList 1 sorted in Descending order (choose option 2 to display sorted list) ..."RESET);
                break;
