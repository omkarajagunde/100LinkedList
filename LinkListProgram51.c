...
int searchAll(SLLNode *headNode, SLLNode *tailNode, int searchElem){
...
void main(void){
	...
	do{
		...
		printf("3] Display total occurence of element? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 2:	displayNodes(&headNode, &tailNode);
				break;
			case 3:
            printf("Enter data to find Occurences ? :");
                scanf("%d",&data);
                retValue = searchAll(headNode, tailNode, data);
                if (retValue == -1)
                    printf(CYN"\t\tData entered was not found ..."RESET);
                else
                    printf(CYN"\t\tTotal occurences of data -> %d"RESET, retValue);
