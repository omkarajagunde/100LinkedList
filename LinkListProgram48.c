...
// This function will the first Ocuurence in CLList ...
int searchFirstOcc(SLLNode **headNode, SLLNode **tailNode, int data){
...
void main(void){
	...
	do{
		...
		printf("3] Search first occurence of element? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 2:	displayNodes(&headNode, &tailNode);
				break;
			case 3:
            printf("Enter data to find first Occurence ? :");
                scanf("%d",&data);
                retValue = searchFirstOcc(&headNode, &tailNode, data);
                if (retValue == -1)
                    printf(CYN"\t\tData entered was not found ..."RESET);
                else
                    printf(CYN"\t\tFisrt occurence of data present at position -> %d"RESET, retValue);
