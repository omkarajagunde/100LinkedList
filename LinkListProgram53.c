...
int concatList(SLLNode **headNodeOne, SLLNode **tailNodeOne, SLLNode **headNodeTwo, SLLNode **tailNodeTwo, int num){
	
	SLLNode *dummyNodeOne = *headNodeOne;
    int count = 1;

    while (count != num){
            dummyNodeOne = dummyNodeOne -> nextNode;
            count ++;
    }

    SLLNode *tempNode = dummyNodeOne -> nextNode;
    *tailNodeOne = dummyNodeOne;
    (*tailNodeOne) -> nextNode = *headNodeOne; 
    free(tempNode);

    (*tailNodeTwo) -> nextNode = *headNodeOne;
    (*tailNodeOne) -> nextNode = *headNodeTwo;
    *tailNodeTwo = *tailNodeOne;
	...
	do{
		...
		switch(choice){
			...
			case 3:	printf("\nCircular Link List One \n: ");
				...
				break;
			case 4:
                printf("Enter how many nodes to concat :");
                scanf("%d",&data);
                retValue = concatList(&headNodeOne, &tailNodeOne,&headNodeTwo, &tailNodeTwo, data);
                if (retValue == 0)
