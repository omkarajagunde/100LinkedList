...
int deleteAtPosition(SLLNode **headNode, int position){
    SLLNode *dummyNode = *headNode;
    SLLNode *tempNode;
	...
	}else if (position == countNodes(&(*headNode))){
			
		while (dummyNode -> nextNode -> nextNode != NULL){
			dummyNode = dummyNode -> nextNode; 
		} 	
	...
	}else if (position > 1 && position < countNodes(&(*headNode))){
		int traverseNodes = 2;
		while(traverseNodes != position){
			dummyNode = dummyNode -> nextNode;
			traverseNodes += 1;
		}

	    tempNode = dummyNode;
		dummyNode -> nextNode = dummyNode -> nextNode -> nextNode;
        dummyNode -> nextNode -> prevNode = dummyNode;
		//free(tempNode);
        tempNode  = NULL;
		return 0;
...
void main(void){
	...
	do{
		...
		printf("3] Delete specified node in DLList 1 ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 3: 
                ...
                scanf("%d",&pos);
                retValue = deleteAtPosition(&headNodeListOne, pos);
                if (retValue == 0)
                    printf(CYN"\t\tspecified Node removed Successfully ..."RESET);
                if (retValue == -1)    
                    printf(CYN"\t\tNo nodes present to Remove ..."RESET);
				break;
