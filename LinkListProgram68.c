...
void main(void){
	...
	do{
		...
		printf("3] Search Second last occurence in DLList 1 ? :\n");
		printf("4] Exit :\n");
		...
		switch(choice){
			...
			case 3: 
                ...
                scanf("%d",&pos);
                retValue = searchSecondLastOcc(headNodeListOne, pos);
                if (retValue == -1)    
                    printf(CYN"\t\tNo nodes present of such data ..."RESET);
                else
                    printf(CYN"\t\tSecond last occurence found @ %d ..."RESET, retValue);
				break;
