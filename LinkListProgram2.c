...
// This function will add a new node to the ending of the nodes ...
int insertLastNode(SLLNode **headNode, int num){

	if ((*headNode) == NULL)
		*headNode = createNode(num);
	else{
		SLLNode *dummyNode = *headNode;
		while (dummyNode->nextNode != NULL){
			dummyNode = dummyNode -> nextNode;
		...
		dummyNode->nextNode = createNode(num);
	}
