...
int swap(SLLNode **srcList){


    if (*srcList == NULL || (*srcList)->nextNode == NULL) 
        return -1; 

    SLLNode *aheadNode = (*srcList) -> nextNode;
    SLLNode *currentNode = *srcList;

    *srcList = aheadNode;
    while (1){

        SLLNode *nNode = aheadNode -> nextNode;
        aheadNode -> nextNode = currentNode;

        if(nNode == NULL || nNode -> nextNode == NULL){
            currentNode -> nextNode =  nNode;
            break;
        }
        
        currentNode -> nextNode = nNode -> nextNode;

        currentNode = nNode;
        aheadNode = currentNode -> nextNode;
    } 
